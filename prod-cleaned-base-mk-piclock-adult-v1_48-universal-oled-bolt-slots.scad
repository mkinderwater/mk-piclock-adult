// mk-piclock-adult V1.48 simplified production model
...
// V1.46 changes the AHT10 mount to a tapered pilot for an M3x5.0 screw.
// V1.48 matches the touch bump peak to the crescent and adds horizontal OLED M2
// slots so one lid STL can tolerate both PLA and measured ASA shrink.
// Units: mm

$fn = 64;

// Select: "base", "lid", "lid_print", "assembly", "touch_crescent_test", "aht10_mount_test", "rubber_foot", or "rubber_feet".
part = "base";

// Set true to flip the whole component layout left-to-right (speaker, touch
// bump, WiFi antenna, AHT10, Pi Zero, amp, feet — everything moves together
// as a rigid mirror, so all existing clearances stay valid). USB-C power is
// the one exception: it switches sides via a plain coordinate swap
...
// Universal PLA/ASA OLED screw clearance. The 156mm modeled X span
// compensates the measured ASA shrink to approximately the 154mm PCB span.
// PLA remains near 156mm, so each screw may need to sit about 1mm inward.
// Keep normal M2 clearance vertically and add 2mm of horizontal slot travel.
oled_m2_clearance_d = 2.7;
oled_m2_slot_extra_x = 2.0;
...
// Tapered printed pilot for an M3x5.0 self-tapping screw. The AHT10 PCB
// already provides a 3.2mm clearance hole for the M3 shank. The wider entry
// guides the screw while the narrower blind end retains thread engagement.
aht10_screw_nominal_d = 3.0;
aht10_screw_length = 5.0;
aht10_pilot_entry_d = 2.9;
aht10_pilot_tip_d = 2.4;
...
assert(
    ...
    "AHT10 M3x5.0 screw leaves insufficient exterior wall shielding."
);
assert(
    aht10_pilot_entry_d > aht10_pilot_tip_d
        && aht10_pilot_entry_d < aht10_screw_nominal_d
        && aht10_pilot_tip_d > 0,
    "AHT10 M3 pilot must taper inward and retain thread engagement."
);
assert(
...
touch_circle_fn = 96;
touch_recess_lid_roof_drop = 1.80;
...
// Round screw-shielding bump over the upper-left touch screw. Keep its peak
// at exactly the same exterior height as the right-side touch crescent. The
// root diameter grows with height to retain the full 45-degree FDM taper.
touch_bump_h = touch_surround_h;
touch_bump_face_d = touch_screw_head_d - 0.20;
touch_bump_root_d = touch_bump_face_d + 2 * touch_bump_h;
touch_bump_slice = 0.20;
touch_bump_fn = 64;
...
touch_bump_root_x0 = touch_bump_x - touch_bump_root_d / 2;
touch_bump_root_x1 = touch_bump_x + touch_bump_root_d / 2;
touch_bump_root_z0 = touch_bump_z - touch_bump_root_d / 2;
touch_bump_root_z1 = touch_bump_z + touch_bump_root_d / 2;

// Printability and screw shielding.
//
// The recess roof climbs from the clipped root chord (near the 1mm sensing
// skin) to the full circle at the face. Its vertical rise is
// touch_recess_lid_roof_drop; its horizontal run is the Y-depth between root
// and face. For a self-supporting (<=45-degree) roof the rise must be at
// least the run, i.e. touch_recess_lid_roof_drop must be >= that depth, not
// <=. (A smaller lid_roof_drop leaves a shallower, harder-to-print roof.)
touch_recess_roof_run =
...
assert(
    touch_recess_lid_roof_drop
        >= touch_recess_roof_run - 0.001,
    "Touch recess roof is shallower than 45 degrees."
);
assert(
...
// External WiFi antenna bore, back wall of the base (z=0) — NOT the lid,
// which is the front (it carries the OLED window). The Pi Zero and amp
// mount directly against this same wall via short standoffs, so this spot
// is chosen left-of-centre, clear of both boards, the speaker, and the
// engraved part number, with margin verified against each below.
wifi_antenna_hole_d = 6.4;
wifi_antenna_x = 65.0;
wifi_antenna_y = 28.0;
wifi_antenna_edge_margin = 15.0;

assert(
    wifi_antenna_x - wifi_antenna_hole_d / 2 >= wifi_antenna_edge_margin
        && wifi_antenna_x + wifi_antenna_hole_d / 2
            <= case_w - wifi_antenna_edge_margin
        && wifi_antenna_y - wifi_antenna_hole_d / 2 >= wifi_antenna_edge_margin
        && wifi_antenna_y + wifi_antenna_hole_d / 2
            <= case_h - wifi_antenna_edge_margin,
    "WiFi antenna hole is too close to the case edge."
);
assert(
    wifi_antenna_x + wifi_antenna_hole_d / 2
        <= center_reinforcement_x - center_floor_rib_w / 2 - 3.0,
    "WiFi antenna hole is too close to the centre floor rib."
);
assert(
    abs(wifi_antenna_y - base_part_number_y)
        >= wifi_antenna_hole_d / 2 + base_part_number_size + 3.0
    || abs(wifi_antenna_x - base_part_number_x) >= 25.0,
    "WiFi antenna hole is too close to the part number text."
);
assert(
    wifi_antenna_x - wifi_antenna_hole_d / 2
        >= speaker_center_x + speaker_w / 2 + 5.0,
    "WiFi antenna hole is too close to the speaker."
);
assert(
    wifi_antenna_x + wifi_antenna_hole_d / 2 <= amp_x - 5.0
        || wifi_antenna_y + wifi_antenna_hole_d / 2 <= amp_y - 5.0,
    "WiFi antenna hole is too close to the amplifier board."
);
assert(
    wifi_antenna_x + wifi_antenna_hole_d / 2 <= pi_x - 5.0
        || wifi_antenna_y + wifi_antenna_hole_d / 2 <= pi_y - 5.0,
    "WiFi antenna hole is too close to the Pi Zero board."
);
...
module oled_m2_horizontal_slot(h, d1, d2 = undef) {
    // A 2mm centre-to-centre hull adds +/-1mm X adjustment while keeping
    // the Y clearance appropriate for an M2 fastener.
    half_shift = oled_m2_slot_extra_x / 2;

    hull() {
        for (sx = [-half_shift, half_shift]) {
            translate([sx, 0, 0])
                if (is_undef(d2)) {
                    cylinder(h = h, d = d1, $fn = 48);
                }
                else {
...
module oled_m2_lid_holes() {
    oled_holes() {
        // Horizontal M2 slot through the lid and solder-joint padding.
        // Total modeled slot size is 4.7mm X by 2.7mm Y. This lets the same
        // STL accommodate the ~154mm ASA result and ~156mm PLA result.
        translate([0, 0, -oled_solder_joint_padding - 1])
            oled_m2_horizontal_slot(
                lid_t + oled_solder_joint_padding + 2,
                oled_m2_clearance_d
            );

        // Match the exterior tapered recess to the horizontal adjustment.
        translate([0, 0, lid_t - oled_m2_recess_depth])
            oled_m2_horizontal_slot(
                oled_m2_recess_depth + 0.3,
                oled_m2_clearance_d,
                oled_m2_recess_d
            );
    }
...
module wifi_antenna_hole() {
    translate([wifi_antenna_x, wifi_antenna_y, -1])
        cylinder(h = wall + 2, d = wifi_antenna_hole_d, $fn = 48);
}
...
module aht10_mount_pilot_hole() {
    translate([
        ...
        rotate([0, -90, 0])
            cylinder(
                h = aht10_pilot_depth,
                d1 = aht10_pilot_entry_d,
                d2 = aht10_pilot_tip_d
            );
}
...
module touch_left_screw_bump() {
    // Radius reduction equals outward projection, retaining a true 45-degree
    // lower surface in the speaker-side-down print orientation.
    hull() {
        touch_bump_slice(
            case_h - 0.40,
            touch_bump_root_d,
